...
def generate_prompts():
    return [
        ("День рождения",
         ...
         "с днём рождения для клиентов этого месяца: в теме письма (не более 40 символов) используй дружелюбную "
         "персонализацию {first_name}; в preheader (≤100) дополни тему и заинтригуй; в теле (≤200 слов) начни "
         "с тёплого обращения {first_name}, поздравь с днём рождения, предложи эксклюзивное преимущество "
         "(скидка {discount} или подарок при заказе), укажи срок действия акции (например, «только до конца месяца»), "
         "вставь яркую CTA-кнопку и закончи подписью команды с контактами"
         ),
        ("Письмо после покупки с кросс-селлом",
         ...
         "В subject (≤40 символов) вырази благодарность и упоминание нового вкуса."
         "В теле письма (до 200 слов) поблагодари {first_name} за заказ {last_order}, кратко охарактеризуй выбранный "
         "сорт, предложи 2–3 новых товара из {recommendations}, подчеркни скидку {discount} на следующий заказ и "
        ...
        ("Активация неактивного клиента",
         ...
         "В subject (≤40 символов) спроси «Мы скучаем, {first_name}?». В теле письма (до 180 слов) "
         "поприветствуй {first_name} тёплым напоминанием о себе, представь подборку лучших хитов продаж, предложи "
         "эксклюзивную скидку {discount}, добавь отзывы клиентов для социального доказательства, и подпишись от лица команды."
         ),
        ("Рождество",
         "Ты — маркетолог в интернет-магазине, специализирующейся на премиальном зерновом кофе, и тебе нужно создать "
         "тёплое рождественское письмо-предложение с предметом (subject) до 40 символов и preheader до 100 символов, "
         "персонализированное с помощью {{first_name}}, где тело письма (до 200 слов) начинается "
         "с поздравления {{first_name}} с Рождеством, рекомендует 3–4 сорта или наборы кофе с описанием вкусовых нот и "
         "способов приготовления, подчёркивает скидку {discount}, указывает срок действия акции (например, до 7 января)"
         ),
        ("Универсальный промпт",
         ...
         "и платформу заказа {order_source}, сформируй персонализированное email-сообщение: Тема письма (до 40 символов) — "
         "коротко и мотивирующе, отражающее индивидуальные предпочтения, и тело письма (до 200 слов) — с обращением по имени, "
         "упоминанием любимой обжарки и метода заваривания, благодарностью за предыдущие заказы, описанием скидки {discount} "
         "и релевантных рекомендаций {recommendations}, призывом к действию с акцентом на выгоду и сроки, а также тёплой подписью команды."
         ),
    ]
